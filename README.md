...
| [Git under the hood](sidelines/git-under-the-hood.md) | Where git came from, what it actually is, and the commands beyond `add`/`commit`/`push`. Three sittings: forty years of prior version control (RCS locks → CVS → SVN → the BitKeeper affair → April 2005) and what distributed buys you; the internals (content-addressed object store, blob/tree/commit/tag, snapshots-not-deltas, the DAG, Merkle chains, the three trees, three-way merge); then the toolbox — `reflog`, `bisect run pytest`, `revert`, `stash`, `rebase -i`, `worktree`, `log -S`, `--force-with-lease` |
