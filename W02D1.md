...
**Goal for today:** finish anything you didn't get to in W01D3 or W01D4, then add a Claude assistant to your toolbox — but on a tight leash.
...
Activate the venv. Run `git log` — you should see your Week 1 commits. Open your W01D3 and W01D4 journals: `subl ~/dev/brand-lens/journal/`.

Be honest with yourself: did you finish W01D3 and W01D4 all the way through? If `fetch.py` doesn't extract the title with BeautifulSoup, or `fetch_many.py` doesn't have try/except + JSON write, you've got catch-up to do. The next two sections are your catch-up. If you finished, skim them as review and move on.

## 2. Catch-up: W01D3 — title and JSON write (~20 min, skip if done)
...
## 3. Catch-up: W01D4 — fetch_many.py (~20 min, skip if done)

If `fetch_many.py` doesn't handle errors, go back to W01D4.md section 4 step C and add the try/except block. The script should run against `urls.txt`, never crash on a bad URL, and produce `results.json` with one record per URL — including the failed ones.
...
Open your W01D3 journal. Find your answer to "what's the difference between `response.text` and `soup.title.string`?" Don't change it yet.
...
Read Claude's answer. Compare it to what you wrote in your journal. **In your W02D1 journal, write down what you got wrong (if anything), and what you now understand differently.**
...
`journal/W02D1.md`. Use this structure from now on:
...
- W02D1 journal includes a "Claude check-in" section
