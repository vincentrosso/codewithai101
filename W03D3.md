...
**Rule for today:** no AI. The mini-quiz at the end is for you to answer from memory and reasoning. Same rule as W02D3.
...
The most common bug: opening with `"w"` when you meant `"a"`. `"w"` silently deletes everything in the file before you write the first byte. If you opened your `results.json` with `"w"` instead of `"a"` on W01D3, you'd have lost every previous run.

W01D3's `fetch.py` used `"a"` to append each fetch as a new line. W03D2's `runner.py` uses `"w"` because it dumps the entire fresh result set every time. Different choices for different jobs.
...
Open `journal/W03D3.md`. Answer each from memory or reasoning.
...
- A `W03D3.md` journal exists with all five quiz answers, written from memory
