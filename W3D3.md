# Week 3, Day 3 — Foundations: File I/O, Paths, and Encodings
*v1.0.0*
...
**Rule for today:** no AI. The mini-quiz at the end is for you to answer from memory and reasoning. Same rule as W2D3.
...
The most common bug: opening with `"w"` when you meant `"a"`. `"w"` silently deletes everything in the file before you write the first byte. If you opened your `results.json` with `"w"` instead of `"a"` on W1D3, you'd have lost every previous run.

W1D3's `fetch.py` used `"a"` to append each fetch as a new line. W3D2's `runner.py` uses `"w"` because it dumps the entire fresh result set every time. Different choices for different jobs.
...
Open `journal/W3D3.md`. Answer each from memory or reasoning.
...
## 9. Optional sideline (whenever you're curious)

If today's "what's actually happening underneath" framing got you wondering how far down it goes — file handles, bytes, encodings, and beyond — there's a sideline that walks the rest of the stack: [sidelines/assembly.md](sidelines/assembly.md). Punch cards, what your Python actually compiles to, why we stopped writing assembly, and where it still matters. Not required, not paced.

---

## What "done" looks like for W3D3

- A `W3D3.md` journal exists with all five quiz answers, written from memory
