# Week 2, Day 3 — Foundations: HTTP, Types, and the Web Beneath the Code
*v1.0.0*

**Goal for today:** no new code. Today is about understanding what's actually happening when your scripts run. The hands-on work of W01D1 through W02D2 gave you patterns; today gives you the model the patterns sit on. Without the model, you'll plateau the moment your scripts get more complicated.
...
You've made HTTP requests, parsed HTML, written to files, looped over lists, called functions. You can do these things, but if someone asked "what is HTTP, actually?" — you'd hesitate. That hesitation is fine on W02D3; it's not fine by Week 6. Today's job is to remove it.
...
That `TypeError` is the one you hit on W01D2. `input()` always returns a string — even if the user types `99`. To compare it to 100 with `>`, you have to convert: `int(number) > 100`. Now you know *why*, not just *that*.

**`None` is a real type.** When `soup.title` doesn't exist on a page, `soup.title` returns `None`, and `None.string` raises `AttributeError`. That's why you wrote `if soup.title else "(no title)"` on W01D3 — to check for None before reaching for `.string`.

## 6. Mini-quiz (~20 min, no AI, no Google)

Open a new file: `journal/W02D3.md`. Answer each question from memory or reasoning. Wrong-and-explained beats right-and-googled.
...
5. Why does `soup.title.string` crash on some pages but not others, and how did you guard against it on W01D3?
...
- A `W02D3.md` journal exists with all five quiz answers, written from memory
...
- You can explain why `int(number)` was needed in your W01D2 script
